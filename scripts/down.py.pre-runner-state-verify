...
def _clear_runner_state_without_container(
    ...
    if volume is None or not _volume_exists(volume):
        print("runner_state volume does not exist; nothing to purge", flush=True)
        return

    mountpoint = _volume_mountpoint(volume)
    if mountpoint is None:
        raise DownError(f"cannot resolve mountpoint for runner_state volume {volume}")

    # Native Docker exposes the volume mountpoint on the host. `sudo make down`
    # has permission to remove these files directly, with zero helper container.
    targets = (
        mountpoint / "submission.json",
        mountpoint / "submission.json.tmp",
        mountpoint / "runner.lock",
    )
    for target in targets:
        try:
            target.unlink()
            print(f"removed runner state: {target.name}", flush=True)
        except FileNotFoundError:
            pass
        except OSError as exc:
            raise DownError(f"cannot remove runner state file {target}: {exc}") from exc
