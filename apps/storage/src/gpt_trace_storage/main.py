...
from .repository import complete_run, fail_run, get_run, set_conversation, start_run, stats
from .schemas import CompleteRunRequest, ConversationRequest, FailRunRequest, RunResponse, StartRunRequest, StatsResponse

app = FastAPI(title="GPT Trace Storage", version="0.1.0")
...
async def start(body: StartRunRequest, session: AsyncSession = Depends(get_session)):
    return await start_run(session, task_id=body.task_id, runner_id=body.runner_id)
...
async def update_conversation(task_id: str, body: ConversationRequest, session: AsyncSession = Depends(get_session)):
    run = await set_conversation(session, task_id=task_id, conversation_id=body.conversation_id)
    if run is None: raise HTTPException(404, "run not found")
    return run
...
async def complete(task_id: str, body: CompleteRunRequest, session: AsyncSession = Depends(get_session)):
    run = await complete_run(session, task_id=task_id, conversation_id=body.conversation_id, messages=body.messages)
    if run is None: raise HTTPException(404, "run not found")
    return run
...
async def fail(task_id: str, body: FailRunRequest, session: AsyncSession = Depends(get_session)):
    run = await fail_run(session, task_id=task_id, error_type=body.error_type, error_message=body.error_message)
    if run is None: raise HTTPException(404, "run not found")
    return run
    ...
    async def rows():
        async with SessionFactory() as session:
            ...
            async for run in result:
                item = {"task_id":run.task_id,"conversation_id":run.conversation_id,
