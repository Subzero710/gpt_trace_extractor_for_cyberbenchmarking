...
class Settings(BaseSettings):
    ...
    chatgpt_poll_interval_seconds: float = Field(default=2.0, gt=0)
    chatgpt_completion_timeout_seconds: float = Field(default=1800.0, gt=0)
    chatgpt_stable_polls: int = Field(default=2, ge=1, le=10)
    chatgpt_upload_settle_seconds: float = Field(default=2.0, ge=0)
    runner_id: str = ""
    ...
    def effective_runner_id(self) -> str:
        return self.runner_id.strip() or f"{socket.gethostname()}-{uuid.uuid4().hex[:8]}"
