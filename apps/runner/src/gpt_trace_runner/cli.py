...
def make_chatgpt(settings: Settings, page) -> ChatGPTClient:
    return ChatGPTClient(
        ...
        stream_start_timeout_seconds=(
            settings.chatgpt_stream_start_timeout_seconds
        ),
        tool_select_timeout_seconds=(
            settings.chatgpt_tool_select_timeout_seconds
        ),
...
def run_command(
    ...
    async def main() -> None:
        ...
        try:
            ...
            chatgpt = make_chatgpt(settings, session.page)
            await chatgpt.goto_home()
            await chatgpt.prompt_box(10_000)
            ...
            runner = BenchmarkRunner(
                ...
                inter_task_delay_seconds=(
                    settings.runner_inter_task_delay_seconds
                ),
                console=console,
            )
            await runner.run(
...
def export_command(
    ...
    async def main() -> None:
        ...
        console.print(
            f"[green]exported {count} runs -> {output}[/]"
        )
