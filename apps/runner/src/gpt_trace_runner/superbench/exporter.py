...
def _row(r):
    clean,n=sanitize(r); runtime=clean.get("runtime_metadata") or {}; apps=clean.get("app_provenance") or []; msgs=normalize_messages(clean.get("messages") or [],used_tool_calls=runtime.get("used_tool_calls") or [],app_provenance=apps); tools=tools_from_provenance(apps)
