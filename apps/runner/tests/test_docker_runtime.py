...
class DockerMock:
    ...
    async def __call__(self, request: httpx.Request) -> httpx.Response:
            ...
            if item is not None:
                if not item["State"]["Running"]:
                    return httpx.Response(
                        409,
                        text="secondary network attach requires a running container",
                        request=request,
                    )
                endpoint = dict(body.get("EndpointConfig") or {})
                endpoint.setdefault(
                    "IPAddress",
                    f"172.30.1.{10 + len(item['NetworkSettings']['Networks'])}",
                )
                item["NetworkSettings"]["Networks"][name] = endpoint
            else:
        ...
        if path == "/containers/create" and method == "POST":
            ...
            address = f"172.30.0.{10 + len(self.containers)}"
            item = {
                "Id": cid,
                "Image": f"sha256:{name}",
...
async def test_attempt_creates_fresh_containers_and_private_networks_then_destroys_everything(tmp_path: Path) -> None:
    ...
    assert attempt.resources["code-workspace"].backend_url.startswith("http://172.30.0.")
    assert attempt.resources["browser"].backend_url.startswith("http://172.30.1.")
    assert "gpt-trace-" not in attempt.resources["code-workspace"].backend_url
    assert "gpt-trace-" not in attempt.resources["browser"].backend_url
...
@pytest.mark.asyncio
async def test_browser_blocked_hosts_are_injected(tmp_path: Path) -> None:
    task = BenchmarkTask("doctor-browser", "prompt", (), (tool("browser"),), None)
    environments = {"browser": "env-b"}
    mock = DockerMock()
    rt = runtime(mock, browser_blocked_hosts=("chatgpt.com",))

    attempt = await rt.create(task, environments, "a" * 64, attempt=1, control_token="s" * 40)
    browser_name = rt._container_name("browser", "env-b")
