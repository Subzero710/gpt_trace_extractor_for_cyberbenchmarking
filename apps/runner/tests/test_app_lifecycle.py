...
async def test_prepare_resume_reset_bind_gateways_to_exact_attempt_runtime(tmp_path: Path) -> None:
    ...
    async def handler(request: httpx.Request):
        if request.url.path == "/control/seed":
            raw = request.content
            calls.append((request.url.host, request.url.path, None, request.headers.get("authorization")))
    ...
    assert any(call[0] == "workspace-gateway" and call[1] == "/control/seed" for call in calls)
