...
class BrowserPolicy:
    def __init__(
        ...
        self.allowed_private_hosts = {
            host.casefold().rstrip(".") for host in (allowed_private_hosts or set())
        }
        self.blocked_hosts = {
            host.casefold().rstrip(".") for host in (blocked_hosts or set()) if host.strip(".")
        }

    def _blocked_host(self, hostname: str) -> bool:
        return any(
    ...
    async def validate_url(self, value: object, *, subresource: bool = False) -> str:
        ...
        if self._blocked_host(hostname):
            raise BrowserAppError(f"blocked host is not allowed: {hostname}")
        try:
...
class BrowserRuntime:
    ...
    async def _launch(self, profile: Path) -> None:
        ...
        binary_override = os.environ.get("CLOAKBROWSER_BINARY_PATH", "").strip()
        if not binary_override:
            raise BrowserAppError(
                "CLOAKBROWSER_BINARY_PATH is required; runtime browser downloads are forbidden"
            )
        binary_path = Path(binary_override)
        if not binary_path.is_file() or not os.access(binary_path, os.R_OK | os.X_OK):
            raise BrowserAppError(
                f"configured CloakBrowser binary is unavailable: {binary_override}"
            )
        environment["CLOAKBROWSER_BINARY_PATH"] = binary_override
