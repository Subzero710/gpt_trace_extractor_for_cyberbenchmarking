...
class ExportFile(M):path:str;target:Literal["browser"]="browser";media_type:str="application/octet-stream"
class ImportFile(M):file_id:str=Field(pattern=r"^f_[0-9a-f]{64}$");path:str;overwrite:bool=False
class FileRef(M):file_id:str;name:str;size:int=Field(ge=0);sha256:str;media_type:str
class ImportOut(M):ok:bool=True;path:str;file_id:str;size:int;sha256:str
...
CAT={'workspace_':'filesystem','terminal':'shell','process':'process','system':'system','environment':'system','current_directory':'system','python':'runtime','system_package':'runtime','git_':'git','http_':'network','download_url':'network','dns_':'network','check_port':'network','workspace_template':'templates','export_file':'filesystem','import_file':'filesystem'}
def cat(n):
