...
def create_app(
    ...
    *,
    ...
    async def seed(request: Request) -> JSONResponse:
        ...
        async with manager.lock:
            if manager.load_state() is not None or any(manager.workspace_root.iterdir()):
                return JSONResponse({"detail": "workspace is not empty before seed"}, status_code=409)
            fd, raw_path = tempfile.mkstemp(
                prefix=".gpttrace-seed-", suffix=".tar", dir=manager.workspace_root
            )
                ...
                with os.fdopen(fd, "wb") as handle:
                    async for chunk in request.stream():
                        ...
                        if total > MAX_SEED_UPLOAD_BYTES:
                            raise WorkspaceError("seed archive is too large")
                        digest.update(chunk)
                        handle.write(chunk)
                    handle.flush()
                    os.fsync(handle.fileno())
                result = manager.seed_archive(archive_path)
                return JSONResponse(
                    {
                        "status": "seeded",
                        "archive_bytes": total,
    ...
    async def control(request: Request) -> JSONResponse:
        ...
        try:
            ...
            LOG.exception("workspace control operation %s failed", operation)
            return JSONResponse(
                {
                    "detail": (
                        f"{operation} internal failure: "
